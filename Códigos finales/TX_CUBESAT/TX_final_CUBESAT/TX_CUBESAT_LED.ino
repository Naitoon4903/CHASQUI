...
bool sendData = false; // Estado de envío de datos
...
void loop() {
  if (rf95.waitAvailableTimeout(1000)) {
    ...
    if (rf95.recv(buf, &len)) {
      ...
      // Cambiar el estado de envío de datos
      sendData = (command == 1);
      digitalWrite(LED_PIN, sendData ? HIGH : LOW);

      // Mostrar los valores recibidos en el monitor serie
      Serial.print("Received command: ");
      Serial.println(command);
    }
      // Enviar datos continuamente si se ha recibido el comando de enviar datos
  /*if (sendData) {
